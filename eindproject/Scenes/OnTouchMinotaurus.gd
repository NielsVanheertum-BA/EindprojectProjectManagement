...
@onready var minotaurus: CharacterBody2D = $".."

var health := BASE_HEALTH


func take_damage() -> void:
	minotaurus.minotaurusHurt = true
	health -= GlobalVariables.sword_damage
	animated_sprite.play("hurt")
	await animated_sprite.animation_finished

	if health <= 0:
		var texture := animated_sprite.sprite_frames.get_frame_texture("hurt", 0)
	...
	minotaurus.minotaurusHurt = false
