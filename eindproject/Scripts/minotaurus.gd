...
var attacking_side := ""
var minotaurusHurt = false

@onready var player: CharacterBody2D = $"../Player"
@onready var animated_sprite: AnimatedSprite2D = $AnimatedSprite2D
@onready var minotaurus_range: Area2D = $Range
...
func _ready() -> void:
	minotaurus_range.area_entered.connect(_on_range_area_entered)
	minotaurus_range.area_exited.connect(_on_range_area_exited)
...
func _physics_process(delta: float) -> void:
	...
	if minotaurusHurt or (is_attacking and is_on_floor()):
		velocity.x = 0
		move_and_slide()
		return
...
func _on_timer_timeout() -> void:
	...
	for body in attack_area.get_overlapping_areas():
		if body.has_method("detect"):
			GlobalVariables.playerCurrentHealth -= GlobalVariables.minotaurusDamage
